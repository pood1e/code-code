...
import { cleanupTestData, apiPost, seedProject, seedMockRunner } from './helpers';
...
test.describe('跨页面数据一致性', () => {
  ...
  test('通过 API 修改 Skill 名称后，刷新页面应展示新名称', async ({ page }) => {
    const skill = await apiPost('/skills', { name: 'Old Name Skill', content: 'content' });
  ...
  test('通过 API 删除 Skill 后，刷新页面不应再显示', async ({ page }) => {
    const skill = await apiPost('/skills', { name: 'Temporary Skill', content: 'content' });

    await page.goto('/skills');
    await expect(page.getByText('Temporary Skill')).toBeVisible();

    await fetch(`http://localhost:3000/api/skills/${skill.id}`, { method: 'DELETE' });
  ...
  test('Profile 中关联的资源名称应与资源列表一致', async ({ page }) => {
    const skill = await apiPost('/skills', { name: 'Consistent Skill', content: 'content' });
...
test.describe('多资源类型交叉验证', () => {
  ...
  test('Skills/Rules/MCPs/Runners 列表页结构应一致（有搜索框和新建按钮）', async ({ page }) => {
  ...
  test('所有资源编辑页应有返回按钮', async ({ page }) => {
    const skill = await apiPost('/skills', { name: 'Back Btn Skill', content: 'content' });
    const rule = await apiPost('/rules', { name: 'Back Btn Rule', content: 'content' });
  ...
  test('编辑页返回按钮应回到列表页', async ({ page }) => {
    const skill = await apiPost('/skills', { name: 'Return Skill', content: 'content' });
