...
func (e catalogProbeLeaseHeldError) Error() string {
	return fmt.Sprintf("platformk8s/modelservice: model catalog probe %q is already running", e.name)
}
...
func (e *CatalogProbeExecutor) acquireCatalogProbeLease(ctx context.Context, key string) (func(), error) {
	...
	lease := &coordinationv1.Lease{
		ObjectMeta: metav1.ObjectMeta{
			Namespace: e.credentialSecretNamespace,
			Name:      name,
		},
	...
	if err := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		...
		if err := e.client.Get(ctx, types.NamespacedName{Namespace: e.credentialSecretNamespace, Name: name}, current); err != nil {
			return err
		}
