...
func (s *Server) runProviderCatalogBinding(ctx context.Context) error {
	if s == nil || s.catalogBinding == nil {
		return fmt.Errorf("platformk8s/providerservice: provider catalog binding is not initialized")
	}
	return s.catalogBinding.SyncAll(ctx)
}
