...
func (s *Server) ResolveProviderCapabilities(ctx context.Context, request *supportv1.ResolveProviderCapabilitiesRequest) (*supportv1.ResolveProviderCapabilitiesResponse, error) {
	...
	case *supportv1.ResolveProviderCapabilitiesRequest_CustomApi:
		return resolveCustomAPICapabilitySubject(subject.CustomApi), nil
...
func (s *Server) resolveProviderCapabilitySubject(ctx context.Context, subject *supportv1.ProviderCapabilitySubject) (*supportv1.ResolveProviderCapabilitiesResponse, error) {
	...
	if providerID == "" && surfaceID == "" {
		return resolveProtocolCapability(protocol, credentialKind), nil
	}
	if cli, ok, err := s.cliByID(ctx, providerID); err != nil {
		return nil, err
	} else if ok {
		return resolveCLICapability(cli, protocol, credentialKind), nil
	}
...
func resolveCLICapability(cli *supportv1.CLI, protocol apiprotocolv1.Protocol, credentialKind credentialv1.CredentialKind) *supportv1.ResolveProviderCapabilitiesResponse {
	...
	return resolveProtocolCapability(protocol, credentialKind)
}

func resolveProtocolCapability(protocol apiprotocolv1.Protocol, credentialKind credentialv1.CredentialKind) *supportv1.ResolveProviderCapabilitiesResponse {
	protocolID := protocolPolicyID(protocol)
	if credentialKind == credentialv1.CredentialKind_CREDENTIAL_KIND_OAUTH {
		return &supportv1.ResolveProviderCapabilitiesResponse{
			EgressPolicyId:         protocolID,
			AuthPolicyId:           protocolID + ".oauth",
			HeaderMetricPolicyId:   protocolID,
...
func resolveCustomAPICapabilitySubject(subject *supportv1.CustomAPICapabilitySubject) *supportv1.ResolveProviderCapabilitiesResponse {
	...
	response := resolveProtocolCapability(protocol, credentialKind)
