...
func (m *SessionManager) StartSession(ctx context.Context, request *credentialv1.OAuthAuthorizationSessionSpec) (*credentialv1.OAuthAuthorizationSessionState, error) {
	...
	if m.providers != nil {
		observer, err := clioauthobservability.Register(m.client, m.namespace, m.providers)
		if err != nil {
			return state, nil
		}
		observer.RecordSessionStarted(request.GetCliId(), request.GetFlow())
	}
	return state, nil
}
...
func (m *SessionManager) CancelSession(ctx context.Context, sessionID string) (*credentialv1.OAuthAuthorizationSessionState, error) {
	...
	if m.providers != nil {
		observer, err := clioauthobservability.Register(m.client, m.namespace, m.providers)
		if err == nil {
			observer.RecordSessionTerminal(resource.Spec.CliID, resource.Spec.Flow, platformv1alpha1.OAuthAuthorizationSessionPhaseCanceled, resource.CreationTimestamp.Time.UTC(), now.Time.UTC())
		}
	}
