...
func TestCredentialManagementServiceListProjectsMaterialStatus(t *testing.T) {
	...
	client := ctrlclientfake.NewClientBuilder().
		...
		WithObjects(testCredentialResource("provider-key", credentialv1.CredentialKind_CREDENTIAL_KIND_API_KEY, 2, []metav1.Condition{{
			...
			Message:            "backing secret missing",
	...
	if got, want := items[0].GetStatus().GetReason(), "backing secret missing"; got != want {
...
func TestCredentialManagementServiceListProjectsOAuthAccountEmailFromSecret(t *testing.T) {
	t.Parallel()
	...
	client := ctrlclientfake.NewClientBuilder().
		...
		WithObjects(
			...
			&corev1.Secret{
				ObjectMeta: metav1.ObjectMeta{Name: credentialID, Namespace: "code-code"},
				Data: map[string][]byte{
					secretKeyAccessToken:  []byte("access-token"),
					secretKeyExpiresAt:    []byte("2026-04-16T12:00:00Z"),
					accountEmailSecretKey: []byte("playerfoxme@gmail.com"),
				},
			},
		).
		Build()
