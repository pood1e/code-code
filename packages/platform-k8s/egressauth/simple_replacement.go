...
func replaceRuleHeader(input ReplacementInput, rule SimpleReplacementRule, headerName string, current string) (string, bool) {
	...
	if rule.Mode == SimpleReplacementModeCookie && normalizeMaterialKey(headerName) == "cookie" && strings.TrimSpace(rule.MaterialKey) == "" && strings.TrimSpace(rule.Template) == "" {
