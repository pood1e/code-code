...
func (s *ManagementService) List(ctx context.Context) ([]*supportv1.Vendor, error) {
	...
	vendorIDs := make([]string, 0, len(staticVendorJSON))
	for vendorID := range staticVendorJSON {
...
func materializeRegisteredVendor(
	...
	raw := staticVendorJSON[strings.TrimSpace(vendorID)]
	if raw == "" {
		return nil, fmt.Errorf("platformk8s: vendor support %q not found", vendorID)
	}
	pkg := &supportv1.Vendor{}
	if err := (protojson.UnmarshalOptions{DiscardUnknown: true}).Unmarshal([]byte(raw), pkg); err != nil {
		return nil, err
	}
