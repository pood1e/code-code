...
func (s *VendorManagementService) List(ctx context.Context) ([]*managementv1.VendorView, error) {
	...
	vendorIDs := make([]string, 0, len(staticVendorDefinitionJSON))
...
func materializeRegisteredVendor(vendorID string) (*vendordefinitionv1.Vendor, error) {
	...
	raw := staticVendorDefinitionJSON[vendorID]
	if raw == "" {
		return nil, fmt.Errorf("platformk8s: vendor definition %q not found", vendorID)
	}
	vendor := &vendordefinitionv1.Vendor{}
	if err := (protojson.UnmarshalOptions{DiscardUnknown: true}).Unmarshal([]byte(raw), vendor); err != nil {
		return nil, err
	}
	if vendor.VendorId == "" {
