...
func NewVendorObservabilityRunner(config VendorObservabilityRunnerConfig) (*VendorObservabilityRunner, error) {
	...
	case config.Client == nil:
		return nil, fmt.Errorf("providerobservability: vendor observability runner client is nil")
	case strings.TrimSpace(config.Namespace) == "":
		return nil, fmt.Errorf("providerobservability: vendor observability runner namespace is empty")
