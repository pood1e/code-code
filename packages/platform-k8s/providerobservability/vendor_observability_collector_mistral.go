...
func (c *mistralVendorObservabilityCollector) Collect(ctx context.Context, input VendorObservabilityCollectInput) (*VendorObservabilityCollectResult, error) {
	...
	if token == "" {
		return nil, unauthorizedVendorObservabilityError("mistral billing: observability token is empty; set observability_credential_ref on the vendor support binding")
	}
