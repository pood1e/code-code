...
| `make kiali-operator-up` | 安装/更新官方 Kiali Operator；当 `infrastructure-addons` 启用 Kiali CR/Route 时必须先运行。 |
| `make infrastructure-addons-up` | 安装/更新 observability addons，并先安装/更新官方 Kiali Operator。 |
...
`infrastructure-addons` 的 Kiali 资源只声明官方 `Kiali` CR 和 Gateway API `HTTPRoute`；`Service/kiali` 与 Deployment 由官方 Kiali Operator 负责生成。启用 `kiali.enabled=true` 或 `kiali.route.enabled=true` 时，使用 `make -C deploy infrastructure-addons-up`，或先运行 `make -C deploy kiali-operator-up` 再单独部署 chart。内网环境可覆盖 `KIALI_OPERATOR_CHART` 和 `KIALI_OPERATOR_IMAGE_REPO` 指向本地 Helm cache / registry cache。
...
- **Kiali 返回 500 / `BackendNotFound`**：先确认 `helm list -A | grep kiali-operator`，再确认 `kubectl -n code-code-observability get svc kiali`。缺失时运行 `make -C deploy kiali-operator-up`，再升级 `infrastructure-addons`。
