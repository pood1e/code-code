...
Kubernetes resources live under `deploy/k8s/notification-dispatcher/base`.
...
Enterprise WeChat egress is allowlisted through the existing Envoy egress gateway by the `platform-network-service` system policy registry. It is a transparent egress rule, so Envoy owns the outlet route without attaching auth header `ext_proc`.
...
- `kustomize build deploy/k8s/notification-dispatcher/base`
- `kubectl apply --dry-run=server -k deploy/k8s/notification-dispatcher/base`
