...
- vendor / API key path 的 API 响应 header 到 metrics 的映射归 vendor package
- CLI-owned OAuth path 的 API 响应 header 到 metrics 的映射可以由 `oauth.observability` 定义
...
- `observability` 可以描述 CLI-owned OAuth path 的 response header collection 与 metric mapping
...
- `VendorCapabilityPackage`
  ...
  - 负责 vendor / API key path 的 API response header -> metric mapping
- `CLISpecializationPackage`
  ...
  - CLI-owned OAuth path 下可以拥有 API response header -> metric mapping
