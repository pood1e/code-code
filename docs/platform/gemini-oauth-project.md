...
- 将 `cloudaicompanionProject.id` 与 tier 名称落到 credential Secret `project_id` / `tier_name`
- 仅作为 Gemini CLI specialized secret data 保存
...
- `account_summary_fields` 可通过 credential-secret source 读取 `project_id` / `tier_name`
