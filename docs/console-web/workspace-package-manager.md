...
- 让 `app`、`packages/*` 与 `../agent-contract` 共享同一 install、build、typecheck、test 入口。
...
- `pnpm-workspace.yaml` 只纳入 `../agent-contract`、`app` 和 `packages/*`；workspace 只有一个 lockfile `packages/console-web/pnpm-lock.yaml`。
